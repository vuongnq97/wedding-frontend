  ...
  return async function apiRequest<T>(path: string, options: ApiRequestOptions = {}): Promise<T> {
    ...
    if (body !== undefined) {
      ...
      if (isFormData) {
        ...
        requestBody = serializeBody(body, requestHeaders.get("Content-Type") ?? "");
      } else {
...
function buildUrl(baseUrl: string | undefined, path: string, query?: ApiRequestOptions["query"]): string {
  ...
  const originFallback = typeof window === "undefined" ? "http://localhost" : window.location.origin;
...
function serializeBody(body: unknown, contentType: string): BodyInit {
  ...
  throw new Error("Unsupported request body type for the provided Content-Type");
}
